...
def register_transcript():
    # Load ACL data from data.json
    acl_data = load_data('data.json')

    # Load transcript data from transcript.json by index
    transcript_index = "1"  # You can dynamically pass this if needed
    transcript_data = load_data('transcript.json')

    # Fetch the transcript using the index
    if transcript_index not in transcript_data:
        return jsonify({"error": "Transcript not found for index"}), 404

    transcript = transcript_data[transcript_index]
    
    student_id = acl_data['student_id']
    acl = acl_data['acl']
    validity = acl['expiration_time']
...
def verify_token():
    ...
    student_address = "0x1E6792b7b75DD5ae09F5F18fF6F18dC8a67709fD"  # Use the student address from registration
    ...
    try:
        ...
        if verified:
            # Load transcript data from transcript.json by index
            transcript_index = "1"  # Set the desired index
            transcript_data = load_data('transcript.json')
            ...
            # Load ACL data for selective disclosure
            acl_data = load_data('data.json')
            selective_disclosure = acl_data['acl'].get('selective_disclosure', [])
